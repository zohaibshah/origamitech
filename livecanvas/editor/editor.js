...
$(document).ready(function($) {
	...
	function lc_set_anchor_text(){
		...
		if(!a) {
			console.log('Something went wrong, no link found');
		...
		if (!lc_url_contains_schema(url)) {
			console.log('url does not contains valid schema');
			url = 'https://' + url;
		}
		*/
...
*/
	...
	$("body").on("click", "#history-steps li", function(e) {
		...
		if (new_html.includes("lc-needs-hard-refresh")) {
			...
			var selector="main";
			previewiframe.contentDocument.document.body.querySelector(selector).outerHTML = doc.querySelector(selector).outerHTML;
   ...
   $("body").on("click", ".customize-colors", function() {
	...
	$("body").on("click", "#sidepanel block", function(e) {
		...
		setPageHTML(selector, new_html); //insert into document

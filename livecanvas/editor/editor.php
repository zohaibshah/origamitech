...
<html data-active-plugins="<?php echo lc_get_active_plugins_list() ?>">
	<head>
		...
		<script>
			...
			lc_editor_fragment_type='<?php  
				if ( get_post_meta($post->ID,'is_header',TRUE)!='' ) echo "header"; 
				if ( get_post_meta($post->ID,'is_footer',TRUE)!='' ) echo "footer";
				?>';
			...
			lc_editor_url_before_editor="<?php
				if('lc_partial' === get_post_type($post->ID)) bloginfo('url');
					elseif (isset($_GET['from_page_edit']))  echo admin_url( 'post.php' )."?action=edit&post=".$post->ID;
	...
	<body>
					...
					<span>Tips & How tos: 
						...
						<select id="lc-editor-tips">
							...
							<optgroup label="Editor">
								<option value="https://github.com/ajaxorg/ace/wiki/Default-Keyboard-Shortcuts">Keyboard shortcuts</option>
		...
		<script defer type='text/javascript' src='https://www.livecanvas.com/remote/lc_update_notification.js?v=<?php echo LC_SCRIPTS_VERSION ?>'></script>
		
		<section id="lc-modal-link" class="lc-modal">
			<?php require_once 'modal-link.php'; ?>
		</section>
	</body>

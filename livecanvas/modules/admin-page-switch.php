...
function lc_add_meta_boxes( $post ){
	add_meta_box( 'lca_meta_boxes', __( 'LiveCanvas', 'livecanvas' ), 'lc_build_meta_box_lc_switch', 'page', 'side', 'high' );
	add_meta_box( 'lca_meta_boxes', __( 'LiveCanvas', 'livecanvas' ), 'lc_build_meta_box_lc_switch', 'post', 'side', 'high' );
	...
	//add the meta box for the LC switch: for any CPT but not LC's own ones. We skip posts and pages as it's already done above
	if (isset($the_post_type) && !in_array($the_post_type, array('page','post','lc_block','lc_section','lc_partial','lc_gt_block'))) 
		add_meta_box( 'lca_meta_boxes', __( 'LiveCanvas', 'livecanvas' ), 'lc_build_meta_box_lc_switch', $the_post_type, 'side', 'high' );
	...
	//add the meta box for shortcode get_post and php example to pull content
	if (isset($the_post_type) && in_array($the_post_type, array('lc_block','lc_section','lc_partial')) )
		add_meta_box( 'lca_meta_boxes', __( 'LiveCanvas', 'livecanvas' ), 'lc_build_meta_box_lc_get_post', $the_post_type, 'side', 'high' );

	//if (isset($currentScreen->id)) add_meta_box( 'lca_meta_boxes', __( 'LiveCanvas', 'livecanvas' ), 'lc_build_meta_box_lc_switch', $currentScreen->id, 'side', 'high' );
}
...
function lc_save_meta_box_data( $post_id ){
	...
	if ( isset( $_REQUEST['livecanvas_enabled'] ) ):
		...
		if ( get_post_meta($post_id, '_lc_livecanvas_enabled', true) ==1 && $_REQUEST['livecanvas_enabled']==1 &&  get_post_type( $post_id ) == 'post'  ) { ///// CASE LC RADIO FROM ON TO ON /////////
...
function lc_build_meta_box_lc_gt_block( $post ){
	...
	<div class='inside'>
		...
		<h2>echo do_shortcode("[lc_get_gt_block id='<?php echo $post->ID; ?>']");</h2>
...
function lc_build_meta_box_lc_get_post( $post ){ 
	...
	<div class='inside'>
		<h4><?php _e( 'You can embed this content using the shortcode:', 'livecanvas' ); ?></h4>
		<h2>[lc_get_post id="<?php echo $post->ID; ?>"]</h2>
		
		<h4><?php _e( 'Alternatively, you can use this PHP code in your templates:', 'livecanvas' ); ?></h4>
		<h2>echo do_shortcode("[lc_get_post id='<?php echo $post->ID; ?>']");</h2>
...
function lc_template_admin_notice_using_lc(){
	...
	<style>
		#postdivrich {display: none} /* so editor is here but hidden, for compatibility with YOAST */
	</style>

...
add_shortcode( 'lc_get_post', function  ( $atts ){
	...
	if (!$the_post) return ("<i>The lc_get_post shortcode could not retrieve the post ID.</i>");
...
add_shortcode( 'lc_get_gt_block', function  ( $atts ){
	...
	$the_post = get_post( $atts['id'] );
	if (!$the_post) return ("<i>The lc_get_gt_block shortcode could not retrieve the post ID.</i>");
